...
#Input
sinal_ = sio.loadmat('sinal') # mudar depois para features
sinal_ = sinal_.get('sinal', 0) # mudar depois para features
#cria uma matriz
sinal = np.zeros(shape = (sinal_.shape[0], 1, sinal_.shape[1], 1)) # este último 1 representa o tipo de imagem RGB, CINZA etc
...
model.add(layers.MaxPooling2D((1, 2)))
model.add(layers.Conv2D(10, (1, 5), activation='relu'))
# droout normalmente entra antes da dense - remove o limiar, ou seja, defino que a baixo de 0.2 os valores serão descartados, por exemplo
